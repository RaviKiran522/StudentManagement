...
function StudentManagement() {
  ...
  let storedStudentsData = localStorage.getItem("students")
storedStudentsData = storedStudentsData ? JSON.parse(storedStudentsData) : students
  const [studentsData, setStudentsData] = useState(storedStudentsData);
  const [filteredStudents, setFilteredStudents] = useState(storedStudentsData);
  const [studentDelete, setStudentDelete] = useState(students);
  ...
  const handledelete = (studentId) => {
    const updatedDetails = studentsData.filter(
      (student) => student.id !== studentId
    );
    setStudentsData(updatedDetails);
    setFilteredStudents(updatedDetails);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Grid>
          <Button variant="contained" onClick={handleSearch} marginRight={"50px"}>
            Search
          </Button> {" "}
          <Button
            variant="contained"
